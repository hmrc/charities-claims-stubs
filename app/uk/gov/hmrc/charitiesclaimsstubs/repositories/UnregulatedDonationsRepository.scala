...
@Singleton
class UnregulatedDonationsRepository @Inject() (
  mongoComponent: MongoComponent
)(implicit ec: ExecutionContext)
    extends PlayMongoRepository[UnregulatedDonation](
      collectionName = "unregulateddonations",
      mongoComponent = mongoComponent,
      domainFormat = UnregulatedDonation.format,
      indexes = Seq(
        IndexModel(Indexes.ascending("charityReference"), IndexOptions().name("charityReferenceIndex").unique(true))
      ),
      extraCodecs = Seq(
        Codecs.playFormatCodec(Format.of[BigDecimal])
      )
    ) {

  def findAll(): Future[Seq[UnregulatedDonation]] =
    ...
    findByCharityReference(charityReference)
      .map {
        ...
        case None =>
          collection
            .insertOne(
              UnregulatedDonation(charityReference, unregulatedDonation)
            )
            .toFuture()
            .map(_ => ())
      }
